...
import AdaFit_multi_scale as DeepFit

from dataset_multi_scale import PointcloudPatchDataset, RandomPointcloudPatchSampler, SequentialShapeRandomPointcloudPatchSampler, SequentialPointcloudPatchSampler
...
def parse_arguments():
    ...
    parser.add_argument('--indir', type=str, default='../pcpnet_new/pclouds/', help='input folder (point clouds)')
    parser.add_argument('--logdir', type=str, default='./log16/', help='training log folder')
...
def train_pcpnet(opt):
    ...
    for epoch in range(opt.nepoch):
        ...
        for train_batchind, data in train_enum:
            ...
            loss, n_loss, _, consistency_loss, normal_loss,bias_loss = compute_loss(
...
def compute_loss(pred, target, outputs, output_pred_ind, output_target_ind, output_loss_weight, normal_loss_type, arch,
    ...
    return loss, n_loss, err_angle, consistency_loss, normal_loss,bias_loss
