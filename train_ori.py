...
import AdaFit_single_scale as DeepFit

from dataset_single_scale import PointcloudPatchDataset, RandomPointcloudPatchSampler, SequentialShapeRandomPointcloudPatchSampler, SequentialPointcloudPatchSampler
...
def parse_arguments():
    ...
    parser.add_argument('--indir', type=str, default='../MSCD_V2/pclouds/', help='input folder (point clouds)')
    parser.add_argument('--logdir', type=str, default='./log3/', help='training log folder')
    ...
    parser.add_argument('--refine', type = bool, default = False, help='flag to refine the model, path determined by outri and model name')
...
def train_pcpnet(opt):
    ...
    for epoch in range(opt.refine_epoch+1 ,opt.nepoch):
        ...
        for train_batchind, data in train_enum:
            ...
            # forward pass
            pred, beta_pred, weights, trans, trans2, neighbor_normals, attn1_weights, attn2_weights = model(points)

            loss, n_loss, _, consistency_loss, normal_loss, tr_rms = compute_loss(
            ...
            log_string('[%s %d: %d/%d] %s loss: %f rms: %f' % (opt.name, epoch,
                                                       train_batchind, train_num_batch-1, green('train'), loss.item(), tr_rms), log_file)
            ...
            np.save((os.path.join(opt.logdir, 'tr_attn2_weights.npy')), np.array(attn2_weights.detach().cpu()))
            ...
            while test_fraction_done <= train_fraction_done and test_batchind+1 < test_num_batch:
                ...
                loss, n_loss, err_angle, consistency_loss, normal_loss, te_rms = compute_loss(
                    pred=pred, target=target,
                    outputs=opt.outputs,
                    output_pred_ind=output_pred_ind,
                    output_target_ind=output_target_ind,
                ...
                # print("rms_te_li: ",rms_te_li)  
                # print("loss_te_li: ",loss_te_li)

                test_fraction_done = (test_batchind+1) / test_num_batch
                avg_test_loss = avg_test_loss + loss.item()
                # print info and update log file
                log_string('[%s %d: %d/%d] %s loss: %f rms: %f' % (opt.name, epoch, train_batchind,
                                                           train_num_batch-1, blue('test'), loss.item(), te_rms), log_file)
        ...
        rms_test_per_ep.append(np.mean(rms_te_li))
        # print("rms test per ep: ",rms_test_per_ep)
        loss_train_per_ep.append(np.mean(loss_tr_li))
        # print("loss train per ep: ",loss_train_per_ep)
        loss_test_per_ep.append(np.mean(loss_te_li))
        # print("loss test per ep: ",loss_test_per_ep)

        #save as np files
        np.save((os.path.join(opt.logdir, 'rms_train_per_ep.npy')), np.array(rms_train_per_ep))
        np.save((os.path.join(opt.logdir, 'rms_test_per_ep.npy')), np.array(rms_test_per_ep))
        np.save((os.path.join(opt.logdir, 'loss_train_per_ep.npy')), np.array(loss_train_per_ep))
        np.save((os.path.join(opt.logdir, 'loss_test_per_ep.npy')), np.array(loss_test_per_ep))
...
def compute_loss(pred, target, outputs, output_pred_ind, output_target_ind, output_loss_weight, normal_loss_type, arch,
    ...
    if trans is not None:
        regularizer_trans += 0.1 * torch.nn.MSELoss()(trans * trans.permute(0, 2, 1),
                                                torch.eye(3, device=trans.device).unsqueeze(0).repeat(
                                                    trans.size(0), 1, 1))
        # print("regularizer trans: ",regularizer_trans)
    if trans2 is not None:
        regularizer_trans += 0.01 * torch.nn.MSELoss()(trans2 * trans2.permute(0, 2, 1),
                                                 torch.eye(64, device=trans.device).unsqueeze(0).repeat(
                                                     trans.size(0), 1, 1))
        # print("regularizae trans2: ", regularizer_trans)
    ...
    for oi, o in enumerate(outputs):
        if o == 'unoriented_normals' or o == 'oriented_normals':
            ...
            if o == 'unoriented_normals':
                ...
                rms = rms_angular_error(o_pred, o_target)
                loss = normal_loss
    ...
    return loss, n_loss, err_angle, consistency_loss, normal_loss, rms
...
def get_model(opt, log_dirname):
    ...
    if opt.arch == 'simple':
        model = DeepFit.DeepFit(1, opt.points_per_patch,
                                            ...
                                            weight_mode=opt.weight_mode, use_consistency=opt.use_consistency, learn_n = opt.learn_n).cuda()
    elif opt.arch == '3dmfv':
        model = DeepFit.DeepFit(1, opt.points_per_patch,
...
#     n_loss = torch.zeros(1, device=pred.device, dtype=pred.dtype)
#     consistency_loss = torch.zeros(1, device=pred.device, dtype=pred.dtype)
#     bias_loss = torch.zeros(1, device=pred.device, dtype=pred.dtype)
    
#     bias_loss = 0.1 * torch.nn.MSELoss()(bias,torch.zeros_like(bias,device=pred.device, dtype=pred.dtype))
#     loss += bias_loss
#     # # generate a inv normal distribution for weight kl div
#     # add pointnet transformation regularization
#     regularizer_trans = 0
...
#     return loss, n_loss, err_angle, consistency_loss, normal_loss,bias_loss, rms
