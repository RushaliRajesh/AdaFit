...
class DeepFit(nn.Module):
    def __init__(self, k=1, num_points=500, use_point_stn=False,  use_feat_stn=False, point_tuple=1,
        ...
        if(self.learn_n):
            self.jet_order = nn.Parameter(torch.nn.init.normal_(torch.empty(4, dtype=torch.float32)))
            print()
            print("learn n: ", self.jet_order)
            print()
    ...
    def forward(self, points):
        ...
        if self.learn_n:
            jet_order = torch.argmax(F.softmax(self.jet_order)) + 1
            print()
            print("in the forward func jet order: ", jet_order)
            print("after softmax: ", F.softmax(self.jet_order, dim=0))
            print("before softma: ", self.jet_order)
...
''' nn.parameter for n-jet vala code below: '''
# import torch
# import torch.nn as nn
# import torch.nn.parallel
# import torch.utils.data
